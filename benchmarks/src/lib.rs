...
impl QueryGenerator {
    ...
    pub fn generate_low_interval_point_queries(total: usize) -> Vec<Query> {
        (0..total).map(|_| Query::point_queries_low_intervals()).collect()
    }
    pub fn generate_high_interval_point_queries(total: usize) -> Vec<Query> {
        (0..total).map(|_| Query::point_queries_high_intervals()).collect()
    }
...
pub fn no_wheeldb_rocks_append_batch(batch: Vec<RideData>, db: &mut wheeldb_rocks::NoWheelDB) {
    for entry in batch {
        db.append(
            entry.pu_location_id.to_le_bytes(),
            Entry::new(entry.fare_amount, entry.do_time),
        );
    }
}
pub fn wheeldb_rocks_append_batch(batch: Vec<RideData>, db: &mut wheeldb_rocks::WheelDB) {
    for entry in batch {
        db.merge(
            entry.pu_location_id.to_le_bytes(),
            Entry::new(entry.fare_amount, entry.do_time),
